...
def plot_top_categories_mean(df,
                            categorical,
                            n,
                            to_plot,
                            figsize=(20,10),
...
def plot_top_categories(df,categorical,n,figsize=(20,10),title=None):
    ...
    categories_top = df.groupby([categorical]).nunique().sort_values(
                        by='admission_id',ascending=False).head(n)[categorical]
    
    df[df[categorical].isin(categories_top.axes[0])].groupby(categorical).agg({
                'admission_id': 'count',
                'length': 'mean'
...
def plot_scatter_length_vs_count(df,dimension,title=None,labeled=False,
                                 xlim=None,ylim=None,figsize=(15,8),N_rows=500):
    
    f, ax = plt.subplots(figsize=figsize)
   
    tmp = df.groupby(dimension).aggregate({
                'admission_id': 'count', 
                'length': 'mean'}).sort_values(
                by='admission_id',
                axis=0,
